...
container_id = subprocess.run("echo {} | sudo -S docker run -it -d -m 256M ".format(pwd)+docker_image+" 2>/dev/null",shell=True, capture_output=True,check=True)
...
execute = subprocess.run("echo {} | sudo -S docker exec ".format(pwd)+container_id + " python3 "+"home"+script_dir[12:]+"/run_py.py "+" /home"+project_dir[12:]+usr_src_code+" /home"+infile[12:]+" /home"+outfile[12:]+" "+str(timelimit)+" /home/" +cli_file[12:],shell=True,capture_output=True)
